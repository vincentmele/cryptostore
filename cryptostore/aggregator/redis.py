...
class Redis(Cache):
    def __init__(self, ip=None, port=None, socket=None, password=None, del_after_read=True, flush=False, retention=None):
        self.del_after_read = del_after_read
        self.retention = retention
        self.last_id = {}
        self.ids = defaultdict(list)
        self.conn = StorageEngines.redis.Redis(ip, port, unix_socket_path=socket, password=password, decode_responses=True)
        if flush:
